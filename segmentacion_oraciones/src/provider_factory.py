...
from segmentacion_oraciones.src.corrector import IdentityCorrectionProvider
from segmentacion_oraciones.src.llm.providers import ClosureProvider, CorrectionProvider
...
def make_closure_provider(
    ...
    *,
    ...
    if name == "linear":
        from segmentacion_oraciones.src.cierre_ml import LinearClosureProvider

        if not model_path:
            raise ValueError("model_path es obligatorio para provider linear")
        return LinearClosureProvider.load(model_path)
    if name == "ollama":
        from segmentacion_oraciones.src.llm.ollama_provider import OllamaProvider
