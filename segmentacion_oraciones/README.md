...
Los contratos viven en `segmentacion_oraciones/src/contracts.py`.
...
interfaces de `segmentacion_oraciones/src/llm/providers.py` sin volver obligatorias esas dependencias.
...
```text
segmentacion_oraciones/outputs/feedback/
segmentacion_oraciones/outputs/llm_logs/
```
...
```bash
python -m segmentacion_oraciones.src.simular_flujo --demo
```

Probar el mismo flujo con cierre via Ollama, solo si Ollama ya esta levantado:

```bash
python -m segmentacion_oraciones.src.simular_flujo --demo --closure-provider ollama --ollama-model qwen3:4b
```

Con archivo propio, una hipotesis parcial por linea:

```bash
python -m segmentacion_oraciones.src.simular_flujo --input ejemplos.txt
```
...
```bash
python -m segmentacion_oraciones.src.evaluar_cierre --demo
```

Con dataset JSONL:

```bash
python -m segmentacion_oraciones.src.evaluar_cierre --input cierre_eval.jsonl
```
...
```bash
python -m segmentacion_oraciones.src.dataset_cierre \
  --split vsr_models/splits/val.csv \
  --limit 30 \
  --output segmentacion_oraciones/outputs/eval/cierre_val.jsonl
```

Luego se evaluan con:

```bash
python -m segmentacion_oraciones.src.evaluar_cierre --input segmentacion_oraciones/outputs/eval/cierre_val.jsonl
```
...
```text
segmentacion_oraciones/GROUND_TRUTH_ORACIONAL.md
segmentacion_oraciones/src/secuencias.py
segmentacion_oraciones/examples/ground_truth_demo.json
segmentacion_oraciones/ground_truth/charla_amor_desamor.json
```
...
python -m segmentacion_oraciones.src.secuencias export-annotation \
  ...
  --limit 40 \
  --output segmentacion_oraciones/outputs/annotation/fuente.md
```
...
python -m segmentacion_oraciones.src.secuencias evaluate \
  --ground-truth segmentacion_oraciones/ground_truth/charla_amor_desamor.json
```
...
```bash
python -m segmentacion_oraciones.src.entrenar_cierre \
  --input segmentacion_oraciones/ground_truth \
  --input C:/Users/bianc/Downloads/synthetic_sentence_boundary_es_ar_v3_curated.zip \
  --output-dir segmentacion_oraciones/outputs/cierre_training
```

El output principal es:

```text
segmentacion_oraciones/outputs/cierre_training/summary.json
segmentacion_oraciones/outputs/cierre_training/best_config.json
segmentacion_oraciones/outputs/cierre_training/*.model.json
```
...
```bash
python -m segmentacion_oraciones.src.simular_flujo \
  --demo \
  --closure-provider linear \
  --model-path segmentacion_oraciones/outputs/cierre_training/linear_heuristic.model.json
```
...
```text
segmentacion_oraciones/GPT_PRO_SYNTHETIC_PROMPT.md
```

Generar un plan de variaciones por lotes:

```bash
python -m segmentacion_oraciones.src.plan_sintetico \
  --shuffle \
  --seed 13 \
  --max 80 \
  --output segmentacion_oraciones/outputs/synthetic_plan/lote_001.jsonl
```
...
- `notebooks/01_segmentacion_oraciones_flujo_y_ground_truth.ipynb`: unifica el contrato de cierre,
...
- `notebooks/03_entrenamiento_llm_cierre.ipynb`: export SFT/chat y plan reproducible
  para entrenar LLM/encoder chico en VM; no inventa resultados si el entorno no esta.

## Tests

```bash
python -m unittest discover -s segmentacion_oraciones/tests
```
