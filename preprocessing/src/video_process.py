...
# Adaptado de Auto-AVSR / Visual_Speech_Recognition_for_Multiple_Languages
#   pipelines/detectors/mediapipe/video_process.py
# Copyright 2023 Imperial College London (Pingchuan Ma)
# Apache 2.0  (http://www.apache.org/licenses/LICENSE-2.0)
#
# Cambios respecto del original:
#   - se quitaron warp_img/apply_transform (usaban skimage y no los usa VideoProcess);
#   - el resto de la logica de alineacion a cara media se mantiene identica.
#
# Que hace: alinea cada cuadro a una "cara media" canonica via transformacion afin
# (usando 4 puntos estables: ojo derecho, ojo izquierdo, nariz, centro de boca) y
# recorta un parche cuadrado centrado en la boca. Es el preprocesamiento que espera
# el modelo de Auto-AVSR.
...
class VideoProcess:
    """Alinea a cara media y recorta el parche labial.

    Espera por cada cuadro 4 puntos en pixeles, en este orden:
        [ojo derecho, ojo izquierdo, punta de nariz, centro de boca]
    (o None si no se detecto cara en ese cuadro; se interpola).
    """
    ...
    def __call__(self, video, landmarks):
        ...
        assert sequence is not None, "cannot crop a patch from the video."
        return sequence
