...
sync_outputs() {
  ...
  gcloud storage cp data_pipeline/release/reports/*.md "$BUCKET/reports/" >/dev/null 2>&1
  set -e
}
...
best_effort_closeout() {
  ...
  if [[ -x ".venv-gpu/bin/python" ]]; then
    .venv-gpu/bin/python data_pipeline/release/scripts/build_full_clean_release_outputs.py
    .venv-gpu/bin/python data_pipeline/release/scripts/validate_clean_bucket.py
  fi
  set -e
}
...
cd "$WORKDIR"
git sparse-checkout set requirements.txt data_pipeline/release preprocessing
git checkout "$BRANCH"
...
mkdir -p data_pipeline/release/reports data_pipeline/release/human_review_pack
gcloud storage cp "$BUCKET/reports/local_source_download_manifest.csv" data_pipeline/release/local_source_download_manifest.csv || true
gcloud storage cp "$BUCKET/argentina/new_discovery/manifests/new_discovery_clip_manifest.csv" data_pipeline/release/new_discovery_clip_manifest.csv || true
gcloud storage cp "$BUCKET/argentina/new_discovery/manifests/new_discovery_asr_manifest.csv" data_pipeline/release/new_discovery_asr_manifest.csv || true
gcloud storage cp "$BUCKET/argentina/new_discovery/manifests/new_discovery_roi_manifest.csv" data_pipeline/release/new_discovery_roi_manifest.csv || true
