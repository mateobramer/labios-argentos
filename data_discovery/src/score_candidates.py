...
from data_discovery.src.common import (
...
from data_discovery.src.estimate_usable_clips import DEFAULT_CLIPS_PER_MINUTE, estimar_clips_aceptados, estimar_minutos_utiles
...
def escribir_shortlist(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for rank, row in enumerate(shortlist, start=1):
        contact = f"data_discovery/outputs/contact_sheets/{row['video_id']}.jpg" if row.get("video_id") else ""
        out.append(
            {
                "rank": rank,
                "url": row["url"],
    ...
    for idx, row in enumerate(accepted[:30], start=1):
        lines.extend(
            [
                ...
                f"- comando tentativo: python descargar_procesar.py \"{row['url']}\"",
                "",
            ]
        )
    lines.extend(
