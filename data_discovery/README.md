...
```bash
python -m data_discovery.src.search_youtube_candidates --max-results 8
```

Auditar samples cortos:

```bash
python -m data_discovery.src.audit_youtube_candidate --limit 20 --sample-count 3 --sample-seconds 24
```

ASR opcional sobre samples (descarga/carga el modelo elegido, CPU/int8):

```bash
python -m data_discovery.src.audit_youtube_candidate --limit 20 --sample-count 3 --sample-seconds 24 --run-asr --asr-model small
```
...
```bash
python -m data_discovery.src.make_contact_sheets --accepted-only
```

Validar:

```bash
python -m compileall data_discovery
python -m unittest discover data_discovery/tests
```
