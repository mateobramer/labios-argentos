...
- Los splits VSR usan 5950 filas en `vsr/splits/splits.csv`.
...
- `vsr/splits/splits.csv` tambien incluye la columna `texto`; ese es el label que
...
Verificado en `vsr/src/fine_tune.py`:

- `--splits-dir`: soportado; default `vsr/splits`.
- `--rois-root`: soportado; requerido.
- `--transcripts-root`: agregado para esta etapa; default vacio, conserva la columna
  `texto` del split.

No se modifican `vsr/splits/{train,val,test}.csv`.
