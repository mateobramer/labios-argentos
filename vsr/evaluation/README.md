...
0. **Setup del modelo** (clona repo + baja checkpoints de Zenodo + entorno + parches):
   `bash vsr/evaluation/setup_modelo_gimeno.sh`. Los pesos (8.5 GB) no se versionan; se bajan ahí.
...
3. **Parches**: `python vsr/evaluation/gimeno_patches/aplicar_parches.py ~/evaluating-end2end-spanish-lipreading`
