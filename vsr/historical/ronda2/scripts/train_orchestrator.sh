...
if [ ! -f "$HOME/.train_setup_done" ]; then
  ...
  bash vsr/evaluation/setup_modelo_gimeno.sh > $HOME/setup_gimeno.log 2>&1 || die "setup_modelo_gimeno fallo (ver setup_gimeno.log)"
...
python -m vsr.src.armar_splits > $HOME/splits.log 2>&1 || die "armar_splits fallo (ver splits.log)"
TR=$(tail -n +2 vsr/splits/train.csv 2>/dev/null | wc -l)
VA=$(tail -n +2 vsr/splits/val.csv 2>/dev/null | wc -l)
TE=$(tail -n +2 vsr/splits/test.csv 2>/dev/null | wc -l)
...
FT(){  # $1=run  $2..=flags extra
  ...
  python -m vsr.src.fine_tune \
    ...
    --rois-root "$ROIS" --out "vsr/runs/$RUN" \
...
FT ft03 > vsr/runs/ft03_train.log 2>&1 || die "ft03 entrenamiento fallo"
log "FT03_DONE (best=$(ls -la vsr/runs/ft03/best.pth 2>/dev/null | awk '{print $5}'))"

# ---- FT04 (config v2: frontend congelado + augment) ----
log "FT04 START (config v2: --freeze frontend --augment)"
FT ft04 --freeze frontend --augment > vsr/runs/ft04_train.log 2>&1 || die "ft04 entrenamiento fallo"
log "FT04_DONE (best=$(ls -la vsr/runs/ft04/best.pth 2>/dev/null | awk '{print $5}'))"
...
eval_one ft03 "$HOME/labios-argentos/vsr/runs/ft03/best.pth"
eval_one ft04 "$HOME/labios-argentos/vsr/runs/ft04/best.pth"
log "EVAL_DONE"
