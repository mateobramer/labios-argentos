...
- **`fine_tune.py` parcheado** para soportar `--freeze`/`--augment` (no existían). Copia en repo local `vsr/src/fine_tune.py` (antes el módulo vivía SOLO en el snapshot) y en la VM (backup `.bak`). Smoke v2 OK.
...
- **Auto-grab de GPU** (background): `bash vsr/historical/ronda2/scripts/autograb_train.sh`.
...
- **OJO: el bucket es de OTRO proyecto (de un compañero) y NO tenemos acceso — ni lectura ni escritura.** Lo confirmé el 2026-06-29: tanto la service account de la VM (`303191263643-compute@...`) como mi cuenta local (`fgutman@udesa.edu.ar`) reciben 403 `storage.objects.get` sobre `lip_rois/`. No puedo arreglarlo con IAM (no soy admin de ese bucket).
  ...
  - **Consecuencia 2 (EVAL):** la EVAL baja `models/ft01_v1/best.pth` y `ft02_v2/best.pth` de GCS para re-evaluarlos sobre test 658 → VA A FALLAR (best-effort, no corta). Para la tabla comparativa v1/v2 vs ft03/ft04 hay que conseguir los WER de v1/v2 de otra fuente. **v1 YA lo tengo registrado: %WER 75.173 ± 2.231, %CER 45.148 ± 1.744** sobre test 658 (estaba en workspace temporal `ft01.wer`). FALTA el de v2 (buscar en resultados locales / pedírselo al compañero). ft03/ft04 sí dan WER absoluto sobre el mismo test 658.
