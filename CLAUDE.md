...
2. `transcribir(video_path, carpeta)` — transcribe con **Whisper** (modelo `small`,
   `language="es"`). Cachea el resultado en `corpus/<titulo>/transcripcion.json`; si ya
   existe, lo reutiliza.
3. `guardar_corpus(resultado, carpeta)` — vuelca el texto limpio (segmentos de ≥3
   palabras) a `corpus/<titulo>/corpus.txt`, una línea por segmento.
4. `cortar_clips(video_path, resultado, carpeta)` — agrupa segmentos en bloques de
   ...
   `clips/<titulo>/clip_NNNN.mp4` + `clip_NNNN.txt` (transcripción limpia del clip).
...
- `videos/<titulo>/` — el `.mp4` original (y a veces `.info.json` de yt-dlp).
- `corpus/<titulo>/` — `transcripcion.json` (salida cruda de Whisper) + `corpus.txt`.
- `clips/<titulo>/` — pares `clip_NNNN.mp4` / `clip_NNNN.txt`.

Hay 3 videos ya procesados (~554 clips). Estos directorios son **datos generados**, no
código; no los edites a mano.

## Dependencias

No hay `requirements.txt`. Se necesita:

- Python: `openai-whisper`, `unidecode`
- Binarios externos en el PATH: `yt-dlp`, `ffmpeg`

⚠️ **Ojo con el PATH de ffmpeg**: `descargar_procesar.py:9` agrega al PATH una ruta
**hardcodeada de Windows** (`C:\Users\Mateo\...`). En macOS/Linux esa línea es inútil
(separador `;`); ffmpeg debe estar disponible globalmente o esa línea hay que
adaptarla. Tenerlo en cuenta al ejecutar fuera de la máquina original.
