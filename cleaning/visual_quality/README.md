...
python -m cleaning.visual_quality.src.detectar_clips_malos

# Una sola fuente
python -m cleaning.visual_quality.src.detectar_clips_malos "<titulo>"

# 2. Materializar el dataset final curado (copia los `keep` -> dataset/)
python -m cleaning.visual_quality.src.detectar_clips_malos --materializar
```
...
python -m cleaning.visual_quality.src.visual_quality_audit \
  ...
  --clips-per-source 2 \
  --max-clips 30 \
  --preview-dir cleaning/visual_quality/outputs/visual_quality_audit/previews
```
...
```bash
python -m cleaning.visual_quality.src.visual_quality_eval_sample \
...
```text
cleaning/visual_quality/notebooks/04_auditoria_visual_offline.ipynb          # resumen/presentacion
cleaning/visual_quality/notebooks/05_diagnostico_politicas_visuales.ipynb   # diagnostico/debug
```
...
`cleaning/visual_quality/src/visual_quality_notebook_utils.py`; los notebooks quedan como capa de
...
```text
cleaning/visual_quality/notebooks/02_analisis_whisper_azzaro.ipynb
```
...
```text
cleaning/visual_quality/outputs/azzaro_whisper/final/
```
...
```text
cleaning/visual_quality/src/llm_text_corrections.py
cleaning/visual_quality/notebooks/03_revision_correcciones_llm.ipynb
```
...
```text
cleaning/visual_quality/outputs/llm_text_corrections/<fuente>/review_webm/
```
