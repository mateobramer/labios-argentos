...
from cleaning.transcript_segmentation.src.cierre import HeuristicClosureProvider
from cleaning.transcript_segmentation.src.corrector import IdentityCorrectionProvider
from cleaning.transcript_segmentation.src.llm.providers import ClosureProvider, CorrectionProvider
...
def make_closure_provider(
    ...
    *,
    ...
    if name == "linear":
        from cleaning.transcript_segmentation.src.cierre_ml import LinearClosureProvider

        if not model_path:
            raise ValueError("model_path es obligatorio para provider linear")
        return LinearClosureProvider.load(model_path)
    if name == "ollama":
        from cleaning.transcript_segmentation.src.llm.ollama_provider import OllamaProvider
