...
Los contratos viven en `cleaning/transcript_segmentation/src/contracts.py`.
...
interfaces de `cleaning/transcript_segmentation/src/llm/providers.py` sin volver obligatorias esas dependencias.
...
```text
cleaning/transcript_segmentation/outputs/feedback/
cleaning/transcript_segmentation/outputs/llm_logs/
```
...
```bash
python -m cleaning.transcript_segmentation.src.simular_flujo --demo
```

Probar el mismo flujo con cierre via Ollama, solo si Ollama ya esta levantado:

```bash
python -m cleaning.transcript_segmentation.src.simular_flujo --demo --closure-provider ollama --ollama-model qwen3:4b
```

Con archivo propio, una hipotesis parcial por linea:

```bash
python -m cleaning.transcript_segmentation.src.simular_flujo --input ejemplos.txt
```
...
```bash
python -m cleaning.transcript_segmentation.src.evaluar_cierre --demo
```

Con dataset JSONL:

```bash
python -m cleaning.transcript_segmentation.src.evaluar_cierre --input cierre_eval.jsonl
```
...
```bash
python -m cleaning.transcript_segmentation.src.evaluar_cierre --input cleaning/transcript_segmentation/outputs/eval/cierre_val.jsonl
```
...
```text
cleaning/transcript_segmentation/GROUND_TRUTH_ORACIONAL.md
cleaning/transcript_segmentation/src/secuencias.py
cleaning/transcript_segmentation/examples/ground_truth_demo.json
cleaning/transcript_segmentation/ground_truth/charla_amor_desamor.json
```
...
```bash
python -m cleaning.transcript_segmentation.src.secuencias evaluate \
  --ground-truth cleaning/transcript_segmentation/ground_truth/charla_amor_desamor.json
```
...
```text
cleaning/transcript_segmentation/outputs/cierre_training/summary.json
cleaning/transcript_segmentation/outputs/cierre_training/best_config.json
cleaning/transcript_segmentation/outputs/cierre_training/*.model.json
```
...
```bash
python -m cleaning.transcript_segmentation.src.simular_flujo \
  --demo \
  --closure-provider linear \
  --model-path cleaning/transcript_segmentation/outputs/cierre_training/linear_heuristic.model.json
```
...
```text
cleaning/transcript_segmentation/GPT_PRO_SYNTHETIC_PROMPT.md
```

Generar un plan de variaciones por lotes:

```bash
python -m cleaning.transcript_segmentation.src.plan_sintetico \
  --shuffle \
  --seed 13 \
  --max 80 \
  --output cleaning/transcript_segmentation/outputs/synthetic_plan/lote_001.jsonl
```
